...
class SmartLanding extends React.Component {
  ...
  _renderComponent = () => {
    ...
    if (!this.props.db.termsConditions && !this.state.firstTime) {
      component = (<TermsConditions startDownload={this._startDowload} ui={this.props.ui} dispatch={this.props.dispatch}/>)
    } else {
      if (this.props.db.isFetching) component = (<ProgressCircle firstFetch={(this.props.db.places.meta.updatedAt === undefined) ? true : false} downloading={true} />)
      else component = (
        <DummyLanding
